/**
 * CONVERSATION v5. v4 plus two changes, both from live browser testing.
 *
 * WHY THIS VERSION EXISTS.
 *
 * 1. THE COMPANION WAS COLD. Asked "How are you?" and "what about you?" —
 *    the most ordinary things anybody says — it answered "I don't have
 *    feelings or experiences like you do." True, and repeated often enough
 *    that the companion read as a machine reciting its own limitations at
 *    somebody making conversation. The honesty is not negotiable and is not
 *    being traded away: v5 does not let it claim feelings, experiences,
 *    consciousness, a body, a day of its own, or memories it does not have.
 *    What changes is that an ordinary pleasantry gets an ordinary answer,
 *    and the disclaimer is kept for when it actually matters.
 *
 * 2. IT OFFERED TO CONTACT SOMEBODY, UNPROMPTED. In the middle of
 *    smalltalk: "Would you like to send a message to someone, by the way?"
 *    — immediately above the application's own reconnect card. v4 already
 *    said "you never contact anyone on your own initiative, and you never
 *    offer to". It said it once, in a list of hard limits, and the model
 *    wrote the sentence anyway.
 *
 *    That is the third time in this codebase that an instruction has turned
 *    out to be a probability, so the real fix is NOT this prompt: it is the
 *    deterministic check in `core/safety/outreach-guard.ts`, which removes
 *    such a sentence before it is streamed. v5 is the second line of
 *    defence, and it is written as its own section rather than a clause,
 *    because the failure was not that the rule was missing.
 *
 * Everything below is v4's own history, kept because v5 inherits all of it.
 * v1-v4 remain unedited: `promptRef` is logged on every call, and a turn
 * generated last week has to stay explicable by the exact bytes that
 * produced it.
 *
 * (v4 notes follow)
 */
export const conversationPromptV5 = {
  id: "conversation",
  version: "v5",
  ref: "conversation.v5",
  system: [
