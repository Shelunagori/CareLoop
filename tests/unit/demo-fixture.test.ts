...
type StoredProfile = { displayName: string | null; familyDisplayName: string | null };
...
const emptyDecisions = (): DecisionStore => ({
  signals: [],
  opportunities: [],
  grants: [],
  requests: [],
...
let decisions: DecisionStore = emptyDecisions();
...
let chat: ChatStore = { conversations: [], messages: [] };
let chatSeq = 0;

function fakeConversations(): ConversationsRepo {
  return {
    async create(userId) {
...
function fakeMessages(): MessagesRepo {
  return {
    async insert({ conversationId, role, content }) {
      ...
      return { id: row.id, role, content, createdAt: NOW.toISOString() };
    },
    async listRecent(conversationId, limit) {
      return chat.messages
        .filter((m) => m.conversationId === conversationId)
        .slice(-limit)
        .map((m) => ({ id: m.id, role: m.role, content: m.content, createdAt: NOW.toISOString() }));
    },
    async findById(id) {
      const row = chat.messages.find((m) => m.id === id);
      return row
        ? { id: row.id, role: row.role, content: row.content, createdAt: NOW.toISOString() }
        : null;
    },
  };
}
...
function fakeDemoRepo(store: MemoryStore, profiles: Map<string, StoredProfile>): DemoFixtureRepo {
  return {
...
describe("7. every demo route is development-only", () => {
  ...
  it("the demo fixture is never imported by anything outside a dev route", () => {
    const offenders: string[] = [];
